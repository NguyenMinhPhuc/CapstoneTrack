...
export default function EarlyInternshipRegistrationPage() {
  ...
  const { data: pastRegistrations, isLoading: isLoadingRegistrations } =
    useCollection<EarlyInternship>(earlyInternshipsQuery);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Nộp và quản lý các đơn đăng ký thực tập sớm của bạn.
            </CardDescription>
          </div>
              ...
              <DialogContent className="sm:max-w-xl">
                ...
                {user && studentData ? (
                  ...
                  <Alert variant="destructive">
                    ...
                    <AlertDescription>
                      Không thể tải hồ sơ sinh viên của bạn. Vui lòng thử lại.
                    </AlertDescription>
                  </Alert>
                )}
              </DialogContent>
        ...
        <CardContent className="space-y-4">
          {!canRegisterNew && (
            <Alert variant="default" className="border-blue-500 text-blue-800">
              ...
              <AlertDescription>
                Bạn hiện có một đơn đăng ký đang ở trạng thái "
                {statusConfig[activeInternship!.status]?.label ||
                  activeInternship!.status}
                ". Bạn cần hoàn thành hoặc hủy đơn đăng ký hiện tại trước khi
                tạo đơn mới.
              </AlertDescription>
            </Alert>
          )}
          {pastRegistrations && pastRegistrations.length > 0 ? (
            <div className="space-y-4">
              {pastRegistrations.map((reg) => {
                ...
                const isRejected =
                  reg.status === "rejected_by_admin" ||
                  reg.status === "rejected_by_company";
                  ...
                  >
                    ...
                    <CardContent className="text-sm space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>
                          {toDate(reg.startDate)
                            ? format(toDate(reg.startDate)!, "dd/MM/yyyy")
                            : "N/A"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <span>
                          {toDate(reg.endDate)
                            ? format(toDate(reg.endDate)!, "dd/MM/yyyy")
                            : "N/A"}
                        </span>
                      </div>
                    </CardContent>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Bạn có chắc chắn muốn hủy đơn đăng ký thực tập
                                tại <strong>{reg.companyName}</strong> không?
            ...
            <Alert>
              ...
              <AlertDescription>
                Bạn chưa có đơn đăng ký thực tập sớm nào. Nhấn "Tạo Đơn đăng ký
                mới" để bắt đầu.
              </AlertDescription>
            </Alert>
          )}
