  ...
  useEffect(() => {
    ...
    const findActiveRegistration = async () => {
        ...
        try {
            ...
            const sessionToSearch = sessionsData.find(s => s.status === 'ongoing') || sessionsData.find(s => s.status === 'upcoming') || null;
    ...
    if (!activeRegistration) {
      return (
        <Alert>
          ...
          <AlertDescription>
            Bạn cần đăng ký đề tài và được giáo viên hướng dẫn xác nhận trước khi có thể nộp thuyết minh.
          </AlertDescription>
        </Alert>
      );
    }
    if (activeRegistration.proposalStatus === 'pending_approval') {
        return (
            <Alert variant="default" className="border-blue-500 text-blue-800">
                ...
                <AlertDescription>
                    Bạn đã nộp thuyết minh thành công. Vui lòng chờ giáo viên hướng dẫn xem xét và duyệt. Bạn vẫn có thể cập nhật lại thông tin nếu cần.
                </AlertDescription>
            </Alert>
        )
    }
    if (activeRegistration.proposalStatus === 'rejected') {
         return (
            <Alert variant="destructive">
                ...
                <AlertDescription>
                    Thuyết minh của bạn đã bị từ chối. Vui lòng xem lại góp ý của giáo viên, chỉnh sửa và nộp lại.
                </AlertDescription>
            </Alert>
        )
    }
      ...
      <div className="max-w-4xl mx-auto">
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Cập nhật chi tiết về đề tài bạn đã đăng ký cho đợt: <strong>{sessionName || '...'}</strong>
                </CardDescription>
            </CardHeader>
            <CardContent>
