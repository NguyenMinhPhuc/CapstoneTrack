...
export function ExportReportButton({
  ...
  const { data: councilMembers } =
    useCollection<DefenseCouncilMember>(councilRef);
  ...
  const { data: subCommittees } =
    useCollection<DefenseSubCommittee>(subCommitteesRef);
  ...
  const { data: registrations } =
    useCollection<DefenseRegistration>(registrationsRef);
  ...
  const generateReport = () => {
    ...
    if (!councilMembers || !subCommittees || !registrations || !evaluations) {
      toast({
        ...
        description:
          "Không thể tải đủ thông tin để tạo báo cáo. Vui lòng thử lại.",
      });
      return;
    }
    ...
    reader.onload = () => {
      try {
        ...
        if (typeof templateContent !== "string") {
          throw new Error("Không thể đọc nội dung file mẫu.");
        }
          ...
          const students = registrations
            ...
            .map((reg) => {
              ...
              const calculateAverage = (
                ...
                const relevantEvals = studentEvals.filter(
                  (e) => e.rubricId === rubricId && e.evaluationType === type,
                );
                if (relevantEvals.length === 0) return null;
                const total = relevantEvals.reduce(
              ...
              return {
                ...
                stt: ++studentCounter,
                scores: {
                  council_grad_avg: calculateAverage(
                    rubricIds.councilGraduation,
                    "graduation",
                  ),
                  council_intern_avg: calculateAverage(
                    rubricIds.councilInternship,
                    "internship",
                  ),
                  supervisor_grad_score: findScore(
                    reg.supervisorId,
                    rubricIds.supervisorGraduation,
                    "graduation",
                  ),
                  company_intern_score: findScore(
                    reg.internshipSupervisorId,
                    rubricIds.companyInternship,
                    "internship",
                  ),
                },
              };
        ...
        const dataForTemplate = {
          ...
          start_date: toDate(session.startDate)
            ? format(toDate(session.startDate)!, "dd/MM/yyyy")
            : "",
          report_date: toDate(session.expectedReportDate)
            ? format(toDate(session.expectedReportDate)!, "dd/MM/yyyy")
            : "",
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Tải lên file mẫu (.docx) của bạn. Hệ thống sẽ điền thông tin từ đợt
            báo cáo này vào file mẫu đó. Hãy chắc chắn file mẫu của bạn có chứa
            các thẻ (placeholder) đúng định dạng.
          </DialogDescription>
        </DialogHeader>
