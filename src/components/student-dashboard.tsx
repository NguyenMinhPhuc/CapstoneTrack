...
const earlyInternshipStatusVariant: Record<
...
export function StudentDashboard({ user }: StudentDashboardProps) {
  ...
  const [activeGraduationRegistration, setActiveGraduationRegistration] =
    useState<DefenseRegistration | null>(null);
  const [activeInternshipRegistration, setActiveInternshipRegistration] =
    useState<DefenseRegistration | null>(null);
  const [activeGraduationSession, setActiveGraduationSession] =
    useState<GraduationDefenseSession | null>(null);
  const [activeInternshipSession, setActiveInternshipSession] =
    useState<GraduationDefenseSession | null>(null);
  ...
  useEffect(() => {
    ...
    const findActiveRegistrations = async () => {
      ...
      try {
        ...
        // Treat registrations that are in 'reporting' OR have been approved by admin
        // as active so the student dashboard reflects confirmed internships.
  ...
  const canSubmitGraduationReport =
    reportSubmission(activeGraduationSession)?.isWindowOpen ||
    settings?.forceOpenReportSubmission;
  const canSubmitInternshipReport =
  ...
  const gradRegStatusKey =
    activeGraduationRegistration?.projectRegistrationStatus || "default";
  const gradRegStatus =
    registrationStatusConfig[gradRegStatusKey] ||
    registrationStatusConfig.default;

  const internRegStatusKey =
    activeInternshipRegistration?.internshipRegistrationStatus || "default";
  const internRegStatus =
    internshipRegStatusConfig[internRegStatusKey] ||
    internshipRegStatusConfig.default;
          ...
          {availableSessions && availableSessions.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Các đợt đang và sắp diễn ra trong năm học.
                </CardDescription>
              </CardHeader>
              <CardContent>
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <div className="flex items-center gap-1.5 mt-1.5">
                              {(session.sessionType === "graduation" ||
                              ...
                              {(session.sessionType === "internship" ||
                                session.sessionType === "combined") && (
          ...
          {activeEarlyInternship && (
            <Card>
              ...
              <CardContent className="space-y-4 text-sm">
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {earlyInternshipProgress.totalHours.toFixed(0)}/
                      {earlyInternshipProgress.goalHours} giờ
                    </p>
                  </div>
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-medium">
                    {activeEarlyInternship.companyName}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-muted-foreground flex items-center gap-1.5">
                  ...
                  <p className="font-medium">
                    {activeEarlyInternship.supervisorName}
                  </p>
                </div>
                <div className="space-y-1">
                  <p className="text-muted-foreground flex items-center gap-1.5">
                  ...
                  <Badge
                    variant={
                      earlyInternshipStatusVariant[
                        activeEarlyInternship.status
                      ] || "secondary"
                    }
                  >
              ...
              <CardFooter className="flex flex-col gap-2">
                ...
                {earlyInternshipProgress.totalHours >=
                  earlyInternshipProgress.goalHours && (
                  <Button asChild className="w-full">
                    <Link
                      href={`/internship-registration?earlyId=${activeEarlyInternship?.id}`}
                    >
                      <FileUp className="mr-2 h-4 w-4" /> Đăng ký Báo cáo Thực
                      tập
                    </Link>
                  </Button>
                )}
                <Button asChild variant="outline" className="w-full">
                  <Link href="/early-internship-registration">
                    <Clock className="mr-2 h-4 w-4" /> Quản lý Thực tập sớm
                  </Link>
                </Button>
              </CardFooter>
            </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Card className="flex flex-col">
              <CardHeader>
                ...
                <CardDescription>
                  Thông tin về đề tài tốt nghiệp trong đợt{" "}
                  <span className="font-semibold">
                    {activeGraduationSession?.name}
                  </span>
                  .
                </CardDescription>
              </CardHeader>
              {activeGraduationRegistration ? (
                  ...
                  <CardContent className="space-y-4 flex-grow">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="space-y-1">
                        ...
                        <p className="font-medium">
                          {activeGraduationRegistration.supervisorName ||
                            "Chưa có"}
                        </p>
                      </div>
                      <div className="space-y-1">
                        ...
                        <Badge variant={gradRegStatus.variant}>
                          {gradRegStatus.label}
                        </Badge>
                      </div>
                    </div>
                    <div className="space-y-3 pt-2">
                      <div className="flex items-center justify-between">
                        <p className="font-medium text-sm">
                          Tiến độ báo cáo tuần
                        </p>
                        <p className="text-sm text-muted-foreground">
                          {weeklyProgress.reportedWeeks}/
                          {weeklyProgress.totalWeeks} tuần
                        </p>
                      </div>
                      <Progress
                        value={
                          (weeklyProgress.reportedWeeks /
                            weeklyProgress.totalWeeks) *
                          100
                        }
                      />
                    </div>
                  ...
                  <CardFooter className="flex flex-col sm:flex-row gap-2">
                    <Button asChild variant="outline" className="w-full">
                      <Link href="/topic-registration">
                        <Book className="mr-2 h-4 w-4" /> Quản lý Đề tài
                      </Link>
                    </Button>
                    <Button asChild variant="outline" className="w-full">
                      <Link href="/progress-report">
                        <Activity className="mr-2 h-4 w-4" /> Báo cáo Tiến độ
                      </Link>
                    </Button>
                  </CardFooter>
                </>
                ...
                <CardContent>
                  <Alert variant="destructive">
                    ...
                    <AlertDescription>
                      Bạn chưa đăng ký báo cáo tốt nghiệp trong đợt này.
                    </AlertDescription>
                  </Alert>
                </CardContent>
              )}
            ...
            <Card className="flex flex-col">
              <CardHeader>
                ...
                <CardDescription>
                  Thông tin về đơn vị thực tập trong đợt{" "}
                  <span className="font-semibold">
                    {activeInternshipSession?.name}
                  </span>
                  .
                </CardDescription>
              </CardHeader>
              {activeInternshipRegistration ? (
                <>
                  <CardContent className="space-y-4 flex-grow">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="space-y-1">
                        <p className="text-muted-foreground flex items-center gap-1.5">
                          <UserCheck /> NHD tại ĐV
                        </p>
                        <p className="font-medium">
                          {activeInternshipRegistration.internship_companySupervisorName ||
                            "Chưa có"}
                        </p>
                      </div>
                      <div className="space-y-1">
                        ...
                        <Badge variant={internRegStatus.variant}>
                          {internRegStatus.label}
                        </Badge>
                      </div>
                    </div>
                  </CardContent>
                  <CardFooter>
                    <Button asChild className="w-full">
                      <Link href="/internship-registration">
                        <Briefcase className="mr-2 h-4 w-4" /> Quản lý Thực tập
                      </Link>
                    </Button>
                  </CardFooter>
                </>
                ...
                <CardContent>
                  <Alert variant="destructive">
                    ...
                    <AlertTitle>Chưa đăng ký</AlertTitle>
                    <AlertDescription>
                      Bạn chưa đăng ký báo cáo thực tập trong đợt này.
                    </AlertDescription>
                  </Alert>
                </CardContent>
              )}
