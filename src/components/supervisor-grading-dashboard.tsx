...
function GraduationGradingView({
  ...
  const projectGroups = useMemo(() => {
    // Supervisors should be able to grade guided groups once students are in reporting flow.
    // Do not hard-require final report approval here, as that can hide valid supervised topics.
  ...
  if (projectGroups.length === 0) {
    return (
      <p className="text-sm text-muted-foreground px-6 pb-4">
        Chưa có sinh viên nào ở trạng thái TN = "Báo cáo"/"Hoàn thành" để chấm
        điểm.
      </p>
    );
  }
...
function InternshipGradingView({
  ...
  // Show students who are in reporting flow.
  ...
  if (validRegistrations.length === 0) {
    return (
      <p className="text-sm text-muted-foreground px-6 pb-4">
        Không có sinh viên nào ở trạng thái TT = "Báo cáo"/"Hoàn thành" để chấm
        điểm.
      </p>
    );
  }
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ĐVTT: {student.internship_companyName || "Chưa có thông tin"}
                </p>
              </div>
              <div className="flex items-center gap-2">
                {evaluation && (
      ...
      {selectedStudent && rubric && (
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="sm:max-w-3xl">
            <GradingForm
              projectGroup={{
                projectTitle: `Thực tập của ${selectedStudent.studentName}`,
...
export function SupervisorGradingDashboard({
  supervisorId,
  userRole,
}: SupervisorGradingDashboardProps) {
  ...
  const [supervisedAssignments, setSupervisedAssignments] = useState<
    SessionAssignments[]
  >([]);
  const [isLoadingAssignments, setIsLoadingAssignments] = useState(true);

  const ongoingSessionIds = useMemo(
  ...
  useEffect(() => {
    ...
    const fetchAssignments = async () => {
      ...
      for (const session of allSessions) {
        ...
        const baseGradQuery = [where("sessionId", "==", session.id)];
        const baseInternQuery = [where("sessionId", "==", session.id)];

        if (userRole === "admin") {
          // Admin sees all students in the session
          gradSupervisorQuery = query(
            collection(firestore, "defenseRegistrations"),
            ...baseGradQuery,
          );
          internSupervisorQuery = query(
            collection(firestore, "defenseRegistrations"),
            ...baseInternQuery,
          );
        } else {
        ...
        const graduationRegistrations = gradSnapshot.docs.map(
          (d) => ({ id: d.id, ...d.data() }) as DefenseRegistration,
        );
        const internshipRegistrations = internSnapshot.docs.map(
          (d) => ({ id: d.id, ...d.data() }) as DefenseRegistration,
        );

        if (
          graduationRegistrations.length > 0 ||
  ...
  const SessionAccordionItem = ({
    sessionData,
  }: {
    sessionData: SessionAssignments;
  }) => {
    const { session, graduationRegistrations, internshipRegistrations } =
      sessionData;

    // Fetch rubrics
    const supGradRubricRef = useMemoFirebase(
      () =>
        session.supervisorGraduationRubricId
          ? doc(firestore, "rubrics", session.supervisorGraduationRubricId)
          : null,
      [firestore, session.supervisorGraduationRubricId],
    );
    const {
      data: supervisorGraduationRubric,
      isLoading: isLoadingSupGradRubric,
    } = useDoc<Rubric>(supGradRubricRef);

    const compInternRubricRef = useMemoFirebase(
      () =>
        session.companyInternshipRubricId
          ? doc(firestore, "rubrics", session.companyInternshipRubricId)
          : null,
      [firestore, session.companyInternshipRubricId],
    );
    const {
      data: companyInternshipRubric,
      isLoading: isLoadingCompInternRubric,
    } = useDoc<Rubric>(compInternRubricRef);
      ...
      <AccordionItem key={session.id} value={`session-${session.id}`}>
        ...
        <AccordionContent className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <GraduationCap /> Đồ án Tốt nghiệp
              </CardTitle>
              <CardDescription className="text-xs">
                Rubric sử dụng:{" "}
                {getRubricName(
                  supervisorGraduationRubric,
                  isLoadingSupGradRubric,
                )}
              </CardDescription>
            </CardHeader>
            {isLoadingRubrics ? (
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                <Briefcase /> Thực tập Doanh nghiệp
              </CardTitle>
              <CardDescription className="text-xs">
                Rubric sử dụng (dành cho đơn vị):{" "}
                {getRubricName(
                  companyInternshipRubric,
                  isLoadingCompInternRubric,
                )}
              </CardDescription>
            </CardHeader>
            {isLoadingRubrics ? (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <Info /> Chưa có sinh viên hướng dẫn
          </CardTitle>
        </CardHeader>
        <CardContent>
          <p>
            Bạn hiện không được phân công hướng dẫn sinh viên nào trong các đợt
  ...
  return (
    <div className="space-y-6">
      <Accordion
        type="multiple"
        defaultValue={supervisedAssignments.map(
