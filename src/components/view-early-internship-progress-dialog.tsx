...
export function ViewEarlyInternshipProgressDialog({
  internship,
  reports,
  goalHours,
  onFinished,
  forceRefresh,
}: ViewEarlyInternshipProgressDialogProps) {
  ...
  const availableWeeks = useMemo(() => {
    ...
    return Array.from({ length: maxWeeks }, (_, i) => i + 1).filter(
  ...
  const handleSubmit = async () => {
    if (newWeekNumber === null || !newHours) {
      toast({
        ...
        description: "Vui lòng chọn tuần và nhập số giờ.",
      });
      return;
    }
  ...
  const handleAction = async (
    ...
    if (
      status === "approved" &&
      (editingHours === "" || isNaN(Number(editingHours)))
    ) {
      toast({
        variant: "destructive",
  ...
  return (
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          Xem và ghi nhận tiến độ hàng tuần cho sinh viên:{" "}
          {internship.studentName} ({internship.studentIdentifier})
        </DialogDescription>
        <div className="text-sm font-semibold pt-2">
          Tổng giờ đã duyệt:{" "}
          <span className="text-primary">
            {totalApprovedHours.toFixed(0)}/{goalHours} giờ
          </span>
        </div>
      </DialogHeader>
      ...
      <div className="py-4 grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <h3 className="text-sm font-medium">
            Tạo Báo cáo (khi SV không nộp)
          </h3>
          <div className="space-y-2">
            <Label htmlFor="week-number">Tuần số</Label>
            <Select
              value={newWeekNumber?.toString() || ""}
              onValueChange={(value) => setNewWeekNumber(Number(value))}
            >
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              placeholder="Nhận xét về công việc của sinh viên trong tuần..."
            />
          </div>
                    ...
                    >
                      ...
                      <AccordionContent className="space-y-4 pt-2">
                        ...
                        <div className="space-y-1">
                          <p className="text-sm">
                            <b>Công việc SV báo cáo:</b> {report.workDone}
                          </p>
                          <p className="text-sm">
                            <b>Kế hoạch tuần tới của SV:</b>{" "}
                            {report.nextWeekPlan}
                          </p>
                        </div>
                        <Separator />
                        <div className="space-y-2">
                          <div className="space-y-1">
                            <Label htmlFor={`approved-hours-${report.id}`}>
                              Số giờ được duyệt
                            </Label>
                            <Input
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            <Button
                              size="sm"
                              onClick={() =>
                                handleAction(report.id, "approved")
                              }
                            >
                              Duyệt
                            </Button>
